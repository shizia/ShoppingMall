...
<div  class="head">
    ...
    <div id="product">
     ...
     <form style="margin-top:30px;" action="<%=basePath%>FoodServlet?action=getFoodByName_user" method="post">
	<label class="form-label col-xs-8 col-xs-8">根据产品名称搜索：</label>
			<div class="formControls col-xs-8 col-sm-8">
				<input type="text" class="input-text" style="width:270px;" value="${searchname}"  name="getbyfoodname">
        ...
        <c:forEach  items="${foodList}" var="food" varStatus="status">
            <div class="product_list">
                <div style="margin-top:20px;" class="img_div">
                    <img class="product_img" alt="" src="${food.foodpic}"/>
                </div>
                <div style="margin-top:20px;" class="product_info">
                    <div style="margin-bottom: 10px;" class="food_name">
                        <span class="sp1">产品:</span>
                        <span class="sp2">${food.foodname}</span>
                    </div>
                  <%--   <div class="food_price">
                        <span class="sp1">描述:</span>
                        <span class="sp2">${food.fooddescribe}</span> 
                    </div> --%>
                    <div class="food_price">
                        <span class="sp1">价格:</span>
                        <span class="sp2">${food.foodprice}</span>
                    </div>
                    <div class="food_amount">
                        <span class="sp1">库存:</span>
                        <span class="sp2">${food.foodtotalnum-food.foodsoldnum}</span>
                    </div>
                    <div class="food_add">
                        <c:if test="${not empty username }">
                         <a style="color:blue; " href="<%=basePath%>VisitRecordServlet?action=visitproduct_record&productid=${food.foodid}&productarea=food">商品详情</a>
                        </c:if>
                        <c:if test="${empty username }">
                         <a style="color:blue; " href="<%=basePath%>VisitRecordServlet?action=visitproduct&productid=${food.foodid}&productarea=food"">商品详情</a>
                        </c:if>
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a style="color:blue; " href="<%=basePath%>CartServlet?action=add&itemid=${food.foodid}">加入购物车</a>
                     </div>
                </div>
            </div>
        </c:forEach>
