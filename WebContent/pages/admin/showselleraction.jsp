...
<div  class="head">
	...
	<div class="htmleaf-container">
		...
		<li><a class="a1" href="<%=basePath%>pages/admin/index.jsp">网站主页</a></li>
			...
			<li style="background-color:black"><a class="a1" href="<%=basePath%>UpdateSellerServlet?action=showallseller_action">销售员操作日志</a></li>
...
<div class="page-container">
	<div style="margin-left:150px;;margin-top:30px;"> 
		<form style="margin-top:30px;" action="<%=basePath%>UpdateSellerServlet?action=getActionByTime" method="post">
	...
	<div>
		<c:if test="${empty selleractionList}">
		<p style="margin-left:650px;margin-top:50px;">无操作记录</p>
		</c:if>
		<c:if test="${not empty selleractionList}">
		<table class="sellproduct" style="width: 80%;margin-left:150px;margin-top:10px;" border="1" >
			...
			<%=request.getAttribute("getByNameError")==null?"":request.getAttribute("getByNameError")%>
				...
				<c:forEach  items="${selleractionList}" var="seller" varStatus="status">
				<tr >
					<td id="sellerid" height="100">${seller.selleridentity}</td>
					<td style="text-align: center;">${seller.sellername}</td>
					<td style="text-align: center;">${seller.selleraction}</td>
					<td style="text-align: center;">${seller.productarea}</td>
					<td style="text-align: center;">${seller.producttype}</td>
					<td style="text-align: center;">${seller.productid}</td>
					<td style="text-align: center;">${seller.productname}</td>
					<td style="text-align: center;">${seller.datetime}</td>
